...
downstream:
	...
	- LC_ALL=C find api/v1beta1 -type f -exec sed -i -- 's/$(UPSTREAM_UPPERCASE)/$(DOWNSTREAM_UPPERCASE)/g' {} +
	- LC_ALL=C find api/v1beta1 -type f -exec sed -i -- 's/$(UPSTREAM_LOWERCASE)/$(DOWNSTREAM_LOWERCASE)/g' {} +
	$(MAKE) generate
	go mod tidy
	go mod vendor
	# fix the cert
