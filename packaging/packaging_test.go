...
func setup() error {
	...
	fmt.Printf("Setting up for packaging tests")
...
func shutdown() {
	fmt.Printf("tearing down for packaging tests")
...
func TestMain(m *testing.M) {
	...
	err := setup()
	if err != nil {
		fmt.Printf("test setup failed: %v", err)
	} else {
		code = m.Run()
	}
	shutdown()
	...
	for _, tt := range getAndRenderManifestTests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := testPackager.getStartEnd(filepath.Join(testPackager.DirCfg.Reports.Path, tt.podReportName)); err != nil {
				if !tt.expectErr {
					testPackager.Log.Info("This error occurred %v", err)
