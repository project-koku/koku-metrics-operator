...
var (
	QueryMap = map[string]string{
		...
		"ros:image_owners":                   "max_over_time(kube_pod_container_info{container!='', container!='POD'}[15m]) * on(pod, namespace) group_left(owner_kind, owner_name) max by(pod, namespace, owner_kind, owner_name) (max_over_time(kube_pod_owner{container!='', container!='POD', pod!='', namespace=~'%s'}[15m]))",
		"ros:image_workloads":                "max_over_time(kube_pod_container_info{container!='', container!='POD'}[15m]) * on(pod, namespace) group_left(workload, workload_type) max by(pod, namespace, workload, workload_type) (max_over_time(namespace_workload_pod:kube_pod_owner:relabel{pod!='', namespace=~'%s'}[15m]))",
