...
var (
	QueryMap = map[string]string{
		...
		"ros:cpu_throttle_container_max":     "max by(container, pod, namespace, node) (max_over_time(rate(container_cpu_cfs_throttled_seconds_total{container!='', container!='POD', pod!=''}[15m])) * on(namespace) group_left (kube_namespace_labels{label_insights_cost_management_optimizations='true', namespace!~'kube-.*|openshift|openshift-.*'} or kube_namespace_labels{label_cost_management_optimizations='true', namespace!~'kube-.*|openshift|openshift-.*'}))",
